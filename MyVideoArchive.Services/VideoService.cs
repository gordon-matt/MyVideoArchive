...
public class VideoService : IVideoService
{
    ...
    public async Task<Result<GetVideoSubtitlesResponse>> GetVideoSubtitlesAsync(int videoId)
    {
        try
        {
            var video = await videoRepository.FindOneAsync(videoId);
            if (video is null || string.IsNullOrEmpty(video.FilePath) || !File.Exists(video.FilePath))
            {
                return Result.NotFound("Video not found");
            }

            var subtitles = DiscoverSubtitleFiles(video.FilePath)
                .Select(x => new VideoSubtitleItem(
                    x.Lang,
                    BuildSubtitleLabel(x.Lang, x.IsAutoGenerated),
                    x.IsAutoGenerated,
                    $"/api/videos/{videoId}/subtitles/{Uri.EscapeDataString(x.Lang)}"))
                .OrderBy(x => x.IsAutoGenerated)
                .ThenBy(x => x.Lang, StringComparer.OrdinalIgnoreCase)
                .ToList();

            return Result.Success(new GetVideoSubtitlesResponse(subtitles));
        }
        catch (Exception ex)
        {
            if (logger.IsEnabled(LogLevel.Error))
            {
                logger.LogError(ex, "Error retrieving subtitles for video {VideoId}", videoId);
            }

            return Result.Error("An error occurred while retrieving subtitles");
        }
    }

    public async Task<Result<SubtitleFileInfo>> GetVideoSubtitleFileAsync(int videoId, string lang)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(lang))
            {
                return Result.Invalid([new ValidationError("lang", "Language code is required")]);
            }
            ...
            var match = DiscoverSubtitleFiles(video.FilePath)
                .FirstOrDefault(x => string.Equals(x.Lang, lang, StringComparison.OrdinalIgnoreCase));

            if (match is null)
            {
                return Result.NotFound("Subtitle track not found");
            }

            return Result.Success(new SubtitleFileInfo(match.FilePath, match.Lang));
        }
        catch (Exception ex)
        {
            if (logger.IsEnabled(LogLevel.Error))
            {
                logger.LogError(ex, "Error retrieving subtitle file {Lang} for video {VideoId}", lang, videoId);
            }

            return Result.Error("An error occurred while retrieving the subtitle file");
        }
    }

    private sealed record DiscoveredSubtitle(string Lang, string FilePath, bool IsAutoGenerated);

    /// <summary>
    /// yt-dlp writes sidecar subtitles as "&lt;id&gt;.&lt;lang&gt;.vtt" next to the video file.
    /// Auto-generated YouTube captions use lang suffixes like "en-orig" (yt-dlp convention).
    /// Custom channel scans convert "&lt;id&gt;.srt" to "&lt;id&gt;.vtt" (no language segment), which we expose as "und".
    /// </summary>
    private static IEnumerable<DiscoveredSubtitle> DiscoverSubtitleFiles(string videoFilePath)
    {
        ...
        // Match "<videoBaseName>.<lang>.vtt"
        var vttFiles = Directory.EnumerateFiles(directory, $"{videoBaseName}.*.vtt", SearchOption.TopDirectoryOnly);
        foreach (string path in vttFiles)
        {
            string name = Path.GetFileName(path);
            string middle = name.Substring(videoBaseName.Length + 1, name.Length - videoBaseName.Length - 1 - ".vtt".Length);
            if (string.IsNullOrEmpty(middle))
            {
                continue;
            }

            bool isAuto = middle.EndsWith("-orig", StringComparison.OrdinalIgnoreCase)
                          || middle.EndsWith(".auto", StringComparison.OrdinalIgnoreCase);

            yield return new DiscoveredSubtitle(middle, path, isAuto);
        }

        // Sidecar from SRT→VTT conversion: "<videoBaseName>.vtt" (same folder as the video)
        string simpleVtt = Path.Combine(directory, $"{videoBaseName}.vtt");
        if (File.Exists(simpleVtt))
        {
            yield return new DiscoveredSubtitle("und", simpleVtt, false);
        }
    }

    private static string BuildSubtitleLabel(string lang, bool isAutoGenerated)
    {
        if (string.Equals(lang, "und", StringComparison.OrdinalIgnoreCase))
        {
            return isAutoGenerated ? "Subtitles (auto)" : "Subtitles";
        }
        ...
        try
        {
            // Normalise e.g. "en-US" / "en-orig" / "en" → "English".
            string baseTag = displayLang.Split('-', '_')[0];
            if (!string.IsNullOrEmpty(baseTag))
            {
                var culture = System.Globalization.CultureInfo.GetCultureInfo(baseTag);
                if (!string.IsNullOrEmpty(culture.EnglishName) && !culture.EnglishName.Contains("Invariant"))
                {
                    displayLang = culture.EnglishName;
                }
            }
        }
        ...
        return isAutoGenerated ? $"{displayLang} (auto)" : displayLang;
    }
